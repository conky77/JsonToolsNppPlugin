...
namespace JSON_Tools.Utils
{
    ...
    public class RandomStringFromRegexTokenizer
    {
        ...
        private int len { get { return Regex.Length; } }
        ...
        /// <param name="dotAll">Equivalent to the 's' flag. if false, '.' matches everything except '\r' and '\n. If true, '.' matches everything.</param>
        /// <returns></returns>
            ...
            while (ii < len)
            {
                ...
                case TokenizerState.CurlybraceQuantifier:
                    if (c == ',')
                    {
                        ...
                        {
                            string quantityStr = Regex.Substring(startOfCurrentState, ii - startOfCurrentState);
                            try
                            {
                ...
                case TokenizerState.ExpectHexChars:
                    if (('0' <= c && c <= '9') || ('a' <= c && c <= 'f') || ('A' <= c && c <= 'f'))
                    {
                        if (ii == endOfHexChars)
                        {
                            int hex = int.Parse(Regex.Substring(startOfCurrentState, ii + 1 - startOfCurrentState), System.Globalization.NumberStyles.HexNumber);
                            if (hex == 0)
                                throw new RandomStringFromRegexCompilerException(ii, RandomStringFromRegexCompilerException.ExceptionType.NulCharacter);
                            if (hex >= 128)
                ...
                case TokenizerState.Flags:
                    ...
                    //case ':':
                    //    // a "(?ON-OFF:...)" construct.
                    //    // I am not going to support this, because it's too complicated.
